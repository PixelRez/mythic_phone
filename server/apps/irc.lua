...
AddEventHandler(
    ...
    function()
        ...
        ESX.RegisterServerCallback(
            "mythic_phone:server:IRCJoinChannel",
            function(source, data, cb)
                local char = exports["mythic_base"]:FetchComponent("Fetch"):Source(source):GetData("character")
                local cData = char:GetData()

                exports["ghmattimysql"]:execute(
                    "INSERT INTO phone_irc_channels (charid, channel) VALUES(@charid, @channel)",
                    {["charid"] = cData.id, ["channel"] = data.channel},
                    function(status)
        ...
        ESX.RegisterServerCallback(
            "mythic_phone:server:IRCLeaveChannel",
            function(source, data, cb)
                local char = exports["mythic_base"]:FetchComponent("Fetch"):Source(source):GetData("character")
                local cData = char:GetData()

                exports["ghmattimysql"]:execute(
                    "DELETE FROM phone_irc_channels WHERE charid = @charid AND channel = @channel",
                    {["charid"] = cData.id, ["channel"] = data.channel},
                    function(status)
                        cb(status.affectedRows > 0)
                    end
        ...
        ESX.RegisterServerCallback(
            "mythic_phone:server:IRCGetMessages",
            function(source, data, cb)
                local char = exports["mythic_base"]:FetchComponent("Fetch"):Source(source):GetData("character")

                exports["ghmattimysql"]:scalar(
                    "SELECT joined FROM phone_irc_channels WHERE charid = @charid AND channel = @channel",
                    {["charid"] = char:GetData("id"), ["channel"] = data.channel},
                    function(joined)
                        if joined ~= nil then
                            exports["ghmattimysql"]:execute(
                                "SELECT * FROM phone_irc_messages WHERE channel = @channel AND date >= FROM_UNIXTIME(@joined)",
                                {["channel"] = data.channel, ["joined"] = (joined / 1000)},
                                function(msgs)
                                    cb(msgs)
                                end
                            )
        ...
        ESX.RegisterServerCallback(
            "mythic_phone:server:IRCNewMessage",
            function(source, data, cb)
                local char = exports["mythic_base"]:FetchComponent("Fetch"):Source(source):GetData("character")

                exports["ghmattimysql"]:scalar(
                    "SELECT channel FROM phone_irc_channels WHERE charid = @charid AND channel = @channel",
                    {["charid"] = char:GetData("id"), ["channel"] = data.channel},
                    function(channel)
                        if channel ~= nil then
                            exports["ghmattimysql"]:execute(
                                "INSERT INTO phone_irc_messages (channel, message) VALUES(@channel, @message)",
                                ...
                                function(res)
                                    if res.affectedRows > 0 then
                                        Citizen.CreateThread(
                                            function()
                                                exports["ghmattimysql"]:execute(
                                                    "SELECT * FROM phone_irc_channels WHERE channel = @channel",
                                                    {["channel"] = data.channel},
                                                    function(data)
                                                        for k, v in ipairs(data) do
                                                            if char:GetData("id") ~= v.charid then
                                                                local tPlayer =
                                                                    exports["mythic_base"]:FetchComponent("Fetch"):CharId(
                                                                    v.charid
                                                                )
                                                                if tPlayer ~= nil then
                                                                    TriggerClientEvent(
